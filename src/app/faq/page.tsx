    ...
    <>
      ...
      <section className="bg-black text-white py-10 md:py-20">
        <div className="container-custom">
          ...
          <div className="max-w-3xl">
            <h1 className="text-2xl md:text-4xl font-bold mb-3">Frequently Asked Questions</h1>
          ...
          <div className="max-w-4xl mx-auto">
            {faqCategories.map((category, categoryIndex) => (
              <div key={category.category} className={categoryIndex > 0 ? 'mt-8 pt-8 border-t' : ''}>
                <h2 className="text-lg md:text-2xl font-bold text-black mb-4">{category.category}</h2>
                <div className="space-y-3">
                  {category.faqs.map((faq, faqIndex) => (
                    <details key={faqIndex} className="bg-[#F5F5F5] rounded-lg p-4 group">
                      <summary className="font-medium text-black cursor-pointer flex items-center justify-between text-sm md:text-base">
                        <span className="pr-4">{faq.question}</span>
                        <svg className="w-4 h-4 text-[#FF8C00] transform group-open:rotate-180 transition-transform flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <section className="py-10 md:py-16 bg-[#F5F5F5]">
        <div className="container-custom text-center">
          <h2 className="text-xl md:text-3xl font-bold text-black mb-3">Still Have Questions?</h2>
